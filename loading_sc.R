...
#Loading function for old test score data
z <- list(1)
fun <- function(x,q){
  for(i in 1:5) {
...
fun2 <- function(x,q){
  for(i in 2:6) {
    z[[i+q]] <<- select(as.data.frame(read_excel(paste(as.character(x)),sheet=i, na = "NA", skip=6))[,1:7],-2)
    
  } 
}
fun("eng6-12.xlsx",0)
fun2("eng13-16.xlsx",4)
fun("math6-12.xlsx",10)
fun2("math13-16.xlsx",14)
...
for(i in 7:13){ 
  if (i < 11 ) {
    x <- as.data.frame(read_excel(paste("", as.character(i), ".xls", sep=""),sheet=1, na = "NA", skip=1))
    valid_column_names <- make.names(names=names(x), unique=TRUE, allow_ = TRUE)
    names(x) <- valid_column_names
    xa <- select(x, DBN,Peer.Index,Grade,Overall.Score,Percentile)
    ...
    valid_column_names <- make.names(names=names(x), unique=TRUE, allow_ = TRUE)
    names(x) <- valid_column_names
    xa <- select(x, DBN,Peer.Index,Overall.Grade,Overall.Score,Percentile)
    names(xa)[2:5] <- c(paste("Peer_", as.character(i),sep = ""),paste("Grade_", as.character(i),sep = ""),paste("Score_", as.character(i),sep = ""),paste("Percentile_", as.character(i),sep = ""))
    acc[[i]] <- xa
  } else {
    x <- as.data.frame(read_excel(paste("", as.character(i), ".xlsx", sep=""),sheet=1, na = "NA", skip=1))
    valid_column_names <- make.names(names=names(x), unique=TRUE, allow_ = TRUE)
    names(x) <- valid_column_names
    xa <- select(x, DBN,Overall.Grade,Overall.Score,Percentile.Rank)
...
for(i in 7:14){ 
  if (i == 7) {
    acc2 <- full_join(acc[[i]], acc[[i+1]], by=c("DBN"))
    
  } else if (i > 9) {
    acc2 <- full_join(acc2, acc[[i-1]], by=c("DBN"))
  }
}
